...
function verifyPassword(password, stored) {
  ...
  try {
    ...
    return (
      storedKey.length === derived.length &&
...
router.post('/register', async (req, res) => {
  ...
  if (!process.env.JWT_SECRET) {
    return res.status(500).json({
      error:
        'Server is not configured. Set JWT_SECRET.'
    });
  }
  ...
  try {
    ...
    if (error.code === '23505') {
      return res.status(409).json({
        error:
          'A shopkeeper account with that phone already exists.'
      });
    }
  ...
  try {
    ...
    if (
      ...
      return res.status(401).json({
        error:
          'Incorrect phone/password or inactive account.'
      });
    }
