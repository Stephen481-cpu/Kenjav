...
function publicShopkeeper(row) {
  return {
    ...
    credit_limit_kes: Number(row.credit_limit_kes),
    is_active: row.is_active,
  };
}

function createShopkeeperToken(shopkeeper) {
...
router.post('/register', async (req, res) => {
  ...
  if (!process.env.JWT_SECRET) {
    return res.status(500).json({
      error: 'Server is not configured. Set JWT_SECRET.',
    });
  }
  ...
  try {
    ...
    const token = createShopkeeperToken(shopkeeper);
    ...
    if (err.code === '23505') {
      return res.status(409).json({
        error: 'A shopkeeper account with that phone already exists.',
      });
    }
...
router.post('/login', async (req, res) => {
  ...
  try {
    ...
    if (
      ...
      !verifyPassword(password, shopkeeper.password_hash)
    ) {
      return res.status(401).json({
        error: 'Incorrect phone/password or inactive account.',
      });
    }
...
router.post(
  '/change-password',
  require('../../middleware/wholesaleAuth').wholesaleAuth,
  require('../../middleware/wholesaleAuth').shopkeeperOnly,
  async (req, res) => {
    ...
    try {
      ...
      console.error('Wholesale password change error:', err);
