...
-- PostgreSQL only.
--
-- Run this against a fresh PostgreSQL/Aiven database.
-- The schema is also written to tolerate some existing tables
-- where practical by using IF NOT EXISTS on tables/columns.
-- ============================================================
...
ALTER TABLE shopkeepers
  ADD COLUMN IF NOT EXISTS credit_limit_kes NUMERIC(12,2) NOT NULL DEFAULT 0;
...
CREATE UNIQUE INDEX IF NOT EXISTS
  idx_wholesale_products_product_unique
...
CREATE TABLE IF NOT EXISTS payments (
  ...
  method VARCHAR(20) NOT NULL DEFAULT 'cash'
    CHECK (method IN ('cash', 'mpesa', 'bank')),
...
ALTER TABLE manual_sales
  ADD COLUMN IF NOT EXISTS shopkeeper_id BIGINT
  REFERENCES shopkeepers(id)
  ON DELETE SET NULL;

ALTER TABLE manual_sales
  ADD COLUMN IF NOT EXISTS wholesale_product_id BIGINT
  REFERENCES wholesale_products(id)
  ON DELETE SET NULL;
...
ALTER TABLE manual_sales
  ADD COLUMN IF NOT EXISTS amount_kes NUMERIC(12,2);
...
ALTER TABLE manual_sales
  ADD COLUMN IF NOT EXISTS sale_period VARCHAR(50);
