...
-- Fresh PostgreSQL schema for the KENJAV retail + wholesale system.
...
CREATE TABLE IF NOT EXISTS products (
  ...
  tag VARCHAR(255), category VARCHAR(255), image_url TEXT,
...
CREATE TABLE IF NOT EXISTS orders (
  ...
  mpesa_checkout_request_id VARCHAR(255), mpesa_merchant_request_id VARCHAR(255),
  mpesa_receipt_number VARCHAR(255), mpesa_phone VARCHAR(50),
  created_at TIMESTAMPTZ NOT NULL DEFAULT CURRENT_TIMESTAMP,
  updated_at TIMESTAMPTZ NOT NULL DEFAULT CURRENT_TIMESTAMP
);
...
CREATE TABLE IF NOT EXISTS purchases (
  id BIGSERIAL PRIMARY KEY,
  shopkeeper_id BIGINT NOT NULL REFERENCES shopkeepers(id) ON DELETE SET NULL,
  product_name TEXT NOT NULL,
  quantity INTEGER NOT NULL CHECK (quantity > 0),
  amount_kes NUMERIC(12,2) NOT NULL CHECK (amount_kes >= 0),
  notes TEXT,
  date TIMESTAMPTZ NOT NULL DEFAULT NOW(),
  source_order_id BIGINT REFERENCES wholesale_orders(id) ON DELETE SET NULL
);
CREATE INDEX IF NOT EXISTS idx_purchases_shopkeeper_date ON purchases(shopkeeper_id, date DESC);
...
CREATE TABLE IF NOT EXISTS payments (
  ...
  date TIMESTAMPTZ NOT NULL DEFAULT NOW(),
  manual_sale_id BIGINT REFERENCES manual_sales(id) ON DELETE CASCADE,
  mpesa_checkout_request_id VARCHAR(255),
  mpesa_merchant_request_id VARCHAR(255),
  mpesa_receipt_number VARCHAR(255),
  mpesa_phone VARCHAR(50)
);
CREATE INDEX IF NOT EXISTS idx_payments_shopkeeper_date ON payments(shopkeeper_id, date DESC);
...
CREATE INDEX IF NOT EXISTS idx_wholesale_expenses_date ON wholesale_expenses(expense_date DESC);
CREATE INDEX IF NOT EXISTS idx_wholesale_expenses_category ON wholesale_expenses(category);
...
CREATE TABLE IF NOT EXISTS inventory_movements (
  ...
  reference_type VARCHAR(50), reference_id BIGINT, notes TEXT,
  created_at TIMESTAMPTZ NOT NULL DEFAULT NOW()
);
